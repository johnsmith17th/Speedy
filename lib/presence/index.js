...
function Presence(app, store) {
	...
	this.on('present', function(presentity, presence) {
...
/**
 * Publish presence.
 * To trigger present event while calling store.publish().
 * 
 * @api public
 */
Presence.prototype.publish = function(key, presence) {
	this.store.publish(key, presence);
